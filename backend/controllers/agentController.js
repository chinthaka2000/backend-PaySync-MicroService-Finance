...
exports.getAgentDashboard = async (req, res) => {
  try {
    ...
    const agentClients = await Client.find({ assignedAgent: agentId });
    ...
    // Loan statistics
    const totalLoans = await Loan.countDocuments({ assignedAgent: agentId });
    ...
    const approvedLoans = await Loan.countDocuments({
      assignedAgent: agentId,
      loanStatus: { $in: ["Approved", "Active"].map(normalizeStatus) },
    });
    ...
    const approvedLoanAmounts = await Loan.aggregate([
      {
        $match: {
          assignedAgent: agentId,
          loanStatus: {
            $in: ["Approved", "Active", "Completed"].map(normalizeStatus),
          },
        },
      },
    ...
    const approvedLoanAmount =
      approvedLoanAmounts.length > 0 ? approvedLoanAmounts[0].totalAmount : 0;
    ...
    // Recent activity
    const recentLoans = await Loan.find({ assignedAgent: agentId })
      .populate("clientUserId", "personalInfo")
      .sort({ createdAt: -1 })
      .limit(5);

    const recentClients = await Client.find({ assignedAgent: agentId })
      .sort({ createdAt: -1 })
      .limit(5);
...
exports.getAgentStats = async (req, res) => {
  try {
    ...
    const activeAgents = await Staff.countDocuments({
      role: "agent",
      status: "active",
    });
...
exports.getAgentClients = async (req, res) => {
  ...
  try {
    ...
    if (search) {
      ...
      clients = await clientRepository.searchClients(
        search,
        { assignedAgent: agentId, ...filters },
        options
      );
    } else {
    ...
    logger.error("Error fetching agent clients", error, {
...
exports.getAgentLoans = async (req, res) => {
  try {
    const agentId = req.params.agentId || req.params.id; // Fix: get agentId from params correctly
    const {
      status,
      workflowStage,
      page = 1,
    ...
    if (workflowStage) filters["workflowState.currentStage"] = workflowStage;
...
exports.getAgentPerformance = async (req, res) => {
  try {
    ...
    const clientStats = await clientRepository.getAgentClientStats(
      agentId,
      dateRange
    );

    // Get loan performance statistics
    const loanStats = await loanRepository.getAgentLoanStats(
      agentId,
      dateRange
    );
      ...
      productivity: {
        ...
        conversionRate:
          loanStats.totalLoans > 0
            ? Math.round(
                (loanStats.totalLoans / clientStats.totalClients) * 100
              )
            : 0,
    ...
    const recentActivity = await Promise.all([
      clientRepository.findByAgent(
        agentId,
        {},
        { limit: 5, sort: { assignedAt: -1 } }
      ),
      loanRepository.findByAgent(
        agentId,
        {},
...
exports.assignClientToAgent = async (req, res) => {
  ...
  try {
    ...
    logger.info("Assigning client to agent", {
      agentId,
      clientId,
...
exports.updateClientInfo = async (req, res) => {
  ...
  try {
    ...
    logger.info("Updating client information", {
      agentId,
      clientId,
    ...
    logger.error("Error updating client information", error, {
      agentId,
...
// Get agent loan statistics with fixed date handling
exports.getAgentLoanStats = async (req, res) => {
  try {
    ...
    // Get additional statistics with proper date handling
    const additionalStats = await Loan.aggregate([
      ...
      },
      {
        $group: {
          ...
          pendingLoans: {
            $sum: {
              $cond: [{ $eq: ["$agentReview.status", "Pending"] }, 1, 0],
            },
          },
          rejectedLoans: {
            $sum: {
              $cond: [{ $eq: ["$agentReview.status", "Rejected"] }, 1, 0],
            },
          },
    ...
    const stats = additionalStats.length > 0 ? additionalStats[0] : {};

    res.json({
      success: true,
      message: "Agent loan statistics fetched successfully",
      data: {
        totalLoans: stats.totalLoans || 0,
        approvedLoans: stats.approvedLoans || 0,
        pendingLoans: stats.pendingLoans || 0,
        rejectedLoans: stats.rejectedLoans || 0,
    ...
    res.status(500).json({
      ...
      error: {
        code: "INTERNAL_ERROR",
        message: "Error getting agent loan statistics",
      },
    });
  }
};
