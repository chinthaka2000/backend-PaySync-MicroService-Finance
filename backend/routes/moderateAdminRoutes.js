...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const existingCEO = await require("../models/Staff").findOne({
        role: "ceo",
      });
      if (existingCEO) {
      ...
      const ceo = new require("../models/Staff")({
        ...
        registrationId: `CEO${Date.now()}${Math.random()
          .toString(36)
          .substr(2, 5)
          .toUpperCase()}`,
        profile: {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const rolePermissions = getPermissionsForRole("regional_manager");
      ...
      const regionalAdmin = new (require("../models/Staff"))({
        ...
        region: regionId,
        assignedDistricts: region.districts,
        createdBy: req.user.userId,
        registrationId: `RM${Date.now()}${Math.random()
          .toString(36)
          .substr(2, 5)
          .toUpperCase()}`,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const regionalManager = await require("../models/Staff")
        .findOne({
      ...
      const agent = new require("../models/Staff")({
        ...
        createdBy: req.user.userId,
        registrationId: `AG${Date.now()}${Math.random()
          .toString(36)
          .substr(2, 5)
          .toUpperCase()}`,
        profile: {
    ...
    try {
      ...
      switch (action) {
        ...
        case "add":
          ...
          break;
        case "remove":
          updatedDistricts = updatedDistricts.filter(
  ...
  async (req, res) => {
    try {
      ...
      if (hasManager !== undefined) {
        query.regionalManager = hasManager === "true" ? { $ne: null } : null;
      }
      ...
      const regions = await require("../models/Region")
        .find(query)
      ...
      const regionsWithStats = await Promise.all(
        regions.map(async (region) => {
          ...
          const agentIds = await require("../models/Staff")
            .find({
              region: region._id,
              role: "agent",
            })
          ...
          const loanCount = await require("../models/Loan").countDocuments({
            clientUserId: {
              $in: await require("../models/Client")
                .find({
                  assignedReviewer: { $in: agentIds },
                })
                .distinct("_id"),
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const region = await require("../models/Region")
        .findById(id)
      ...
      const clientIds = await require("../models/Client")
        .find({
          assignedReviewer: { $in: agentIds },
        })
        .distinct("_id");

      const [staffStats, clientStats, loanStats, recentActivity] =
        await Promise.all([
      ...
      res.status(200).json({
        ...
        data: {
          ...
          recentActivity,
          performance: {
            staffUtilization: staffStats.reduce(
              (acc, stat) => acc + stat.active,
              0
            ),
            totalClients: clientStats.reduce(
              (acc, stat) => acc + stat.count,
              0
            ),
            totalLoanAmount: loanStats.reduce(
              (acc, stat) => acc + stat.totalAmount,
              0
            ),
          },
        },
...
router.get(
  ...
  validate(
    Joi.object({
      ...paginationSchema,
      role: Joi.string()
        .valid("agent", "regional_manager", "moderate_admin", "ceo")
        .optional(),
  ...
  async (req, res) => {
    try {
      ...
      if (hasManager !== undefined) {
        query.managedBy = hasManager === "true" ? { $ne: null } : null;
      }
      ...
      const staffWithMetrics = await Promise.all(
        staff.map(async (member) => {
          ...
          if (member.role === "agent") {
            ...
            const clientCount =
              await require("../models/Client").countDocuments({
