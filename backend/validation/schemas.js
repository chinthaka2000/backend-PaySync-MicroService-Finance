...
const paginationSchema = {
  ...
  sort: Joi.string().valid('createdAt', '-createdAt', 'loanAmount', '-loanAmount', 'name', '-name').default('-createdAt')
};
...
const authSchemas = {
  ...
  validateRoleCreation: {
    body: Joi.object({
      targetRole: Joi.string().valid('agent', 'regional_manager', 'moderate_admin', 'ceo', 'super_admin').required()
    })
  }
};
...
const loanSchemas = {
  createLoan: {
    body: Joi.object({
      ...
      collateral: Joi.object({
        type: Joi.string().valid('property', 'vehicle', 'jewelry', 'other').required(),
  ...
  getAgentLoans: {
    ...
    query: Joi.object({
      ...paginationSchema,
      status: Joi.string().valid('pending', 'approved', 'rejected', 'active', 'completed', 'defaulted').optional(),
  ...
  updateLoanStatus: {
    ...
    body: Joi.object({
      status: Joi.string().valid('pending', 'approved', 'rejected', 'active', 'completed', 'defaulted').required(),
  ...
  regionalApproval: {
    ...
    body: Joi.object({
      ...
      conditions: Joi.array().items(Joi.string().min(5).max(200)).optional(),
      recommendedAmount: Joi.number().positive().optional(),
      approvalLevel: Joi.string().valid('standard', 'conditional', 'high_value').optional()
    })
  },
  ...
  getRegionalManagerLoans: {
    ...
    query: Joi.object({
      ...paginationSchema,
      status: Joi.string().valid('pending_approval', 'approved', 'rejected', 'all').default('pending_approval'),
      search: Joi.string().min(2).max(100).optional(),
      sortBy: Joi.string().valid('createdAt', 'agentReview.reviewDate', 'loanAmount', 'clientName').default('agentReview.reviewDate'),
  ...
  getEnhancedAgentLoans: {
    ...
    query: Joi.object({
      ...
      status: Joi.alternatives().try(
        Joi.string().valid('pending', 'approved', 'rejected', 'active', 'completed', 'defaulted', 'under_review'),
        Joi.array().items(Joi.string().valid('pending', 'approved', 'rejected', 'active', 'completed', 'defaulted', 'under_review'))
      ).optional(),
      workflowStage: Joi.string().valid(
        'application_submitted', 'agent_approved', 'agent_rejected',
        'regional_approved', 'regional_rejected', 'agreement_generated',
        'funds_disbursed', 'loan_active', 'loan_completed'
      ).optional(),
      search: Joi.string().min(2).max(100).optional(),
      sortBy: Joi.string().valid('createdAt', 'updatedAt', 'loanAmount', 'clientName', 'status').default('createdAt'),
...
const clientSchemas = {
  registerClient: {
    body: Joi.object({
      ...
      employmentInfo: Joi.object({
        employmentType: Joi.string().valid('employed', 'self_employed', 'unemployed', 'retired').required(),
        employer: Joi.string().min(2).max(100).when('employmentType', {
          is: Joi.valid('employed'),
          then: Joi.required(),
          otherwise: Joi.optional()
        }),
        jobTitle: Joi.string().min(2).max(100).when('employmentType', {
          is: Joi.valid('employed'),
          then: Joi.required(),
          otherwise: Joi.optional()
        }),
  ...
  getClientByAssignerId: {
    ...
    query: Joi.object({
      ...paginationSchema,
      status: Joi.string().valid('pending', 'approved', 'rejected', 'suspended', 'active').optional()
    })
  },
  ...
  updateClientStatus: {
    body: Joi.object({
      ...
      status: Joi.string().valid('pending', 'approved', 'rejected', 'suspended', 'active').required(),
...
const staffSchemas = {
  createStaff: {
    body: Joi.object({
      ...
      role: Joi.string().valid('agent', 'regional_manager', 'moderate_admin', 'ceo', 'super_admin').required(),
...
const paymentSchemas = {
  recordPayment: {
    body: Joi.object({
      ...
      amount: Joi.number().positive().required(),
      paymentDate: Joi.date().iso().default(() => new Date()),
      paymentMethod: Joi.string().valid('cash', 'bank_transfer', 'mobile_money', 'check', 'online').required(),
      reference: Joi.string().min(3).max(50).optional(),
  ...
  getPaymentHistory: {
    ...
    query: Joi.object({
      ...
      startDate: Joi.date().iso().optional(),
      endDate: Joi.date().iso().min(Joi.ref('startDate')).optional(),
      paymentMethod: Joi.string().valid('cash', 'bank_transfer', 'mobile_money', 'check', 'online').optional()
    })
  }
};
...
const reportSchemas = {
  generateReport: {
    body: Joi.object({
      reportType: Joi.string().valid('loan_summary', 'payment_history', 'agent_performance', 'regional_overview', 'financial_summary').required(),
      startDate: Joi.date().iso().required(),
      endDate: Joi.date().iso().min(Joi.ref('startDate')).required(),
      filters: Joi.object({
        regionId: objectId.optional(),
        agentId: objectId.optional(),
        loanStatus: Joi.string().valid('pending', 'approved', 'rejected', 'active', 'completed', 'defaulted').optional(),
        minAmount: Joi.number().positive().optional(),
        maxAmount: Joi.number().positive().min(Joi.ref('minAmount')).optional()
      }).optional(),
